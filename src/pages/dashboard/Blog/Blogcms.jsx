...
const Blogcms = () => {
  ...
  async function fetchBlogs() {
    ...
    try {
      ...
      Swal.fire('Error', `Failed to fetch blogs: ${err.message}`, 'error')
    } finally {
      setLoading(false)
    }
  }
  ...
  async function openEditDialog(blog) {
    ...
    setImageFile(null)
    setImagePreview(blog.image ? `https://testapicms.pvorasp.com/api/uploads/${blog.image}` : null)
    setDialogOpen(true)
  }
  ...
  async function handleSave() {
    if (!formData.title.trim() || !formData.description.trim() || !formData.blogDate) {
    ...
    try {
      ...
      if (imageFile) {
        form.append('blogImage', imageFile)
      }
      ...
      Swal.fire('Success', result.message || (editingId ? 'Blog updated successfully' : 'Blog added successfully'), 'success')
      closeDialog()
      await fetchBlogs()
    } catch (err) {
      Swal.fire('Error', `Failed to save blog: ${err.message}`, 'error')
    }
  }
  ...
  async function handleDelete(id) {
    ...
    try {
      ...
      Swal.fire('Deleted', result.message || 'Blog deleted successfully', 'success')
      await fetchBlogs()
    } catch (err) {
  ...
  async function handleStatusToggle(id, currentStatus) {
    try {
      const res = await apiFetch(`/blogs/${id}/status`, {
        method: 'PATCH',
      ...
      Swal.fire('Success', result.message || 'Blog status updated successfully', 'success')
      await fetchBlogs()
    } catch (err) {
      Swal.fire('Error', `Failed to update blog status: ${err.message}`, 'error')
    }
  }
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} md={6}>
              <TextField
                ...
                margin="normal"
                placeholder="e.g., Five Business Tips"
              />
            </Grid>
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={(e) => handleFormChange('blogDate', e.target.value)}
                margin="normal"
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={(e) => handleFormChange('blogBy', e.target.value)}
                margin="normal"
            ...
            <Grid item xs={12}>
              <TextField
                ...
                multiline
                minRows={4}
                placeholder="e.g., <p>Blog content here...</p>"
              />
            </Grid>
              ...
              <TableBody>
                {blogs.map((blog) => (
                  <TableRow key={blog.id} hover>
                    ...
                    <TableCell>{blog.blogBy}</TableCell>
                    <TableCell>{new Date(blog.blogDate).toLocaleDateString()}</TableCell>
                    <TableCell>
                      {blog.image ? (
                        <img
                          src={`https://testapicms.pvorasp.com/api/uploads/${blog.image}`}
                          alt={blog.title}
                    ...
                    <TableCell>
                      <Button
                        size="small"
                        variant="outlined"
                        color={blog.status === 1 ? 'success' : 'error'}
