...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Try to extract token from several possible response shapes
      const token =
        response.data?.token ||
      ...
      if (token) {
        // Save token in multiple keys for compatibility
        try {
        ...
        // Optional: Save token expiry time (if provided) or fallback to 24h
        const expiryFromResponse = response.data?.expiresAt || response.data?.data?.expiresAt || null;
        const expiryTime = expiryFromResponse
          ? new Date(expiryFromResponse).getTime()
          : new Date().getTime() + 24 * 60 * 60 * 1000;
        localStorage.setItem("tokenExpiry", expiryTime.toString());

        setSuccess("Login successful! Redirecting...");

        // Play voice notification using Web Speech API with user gesture
        const playVoiceNotification = () => {
          try {
            ...
            utterance.text = "Login successfully. Welcome to Pihora CMS";
            utterance.rate = 0.9;
            utterance.pitch = 1.5;
            utterance.volume = 1;
            ...
            if (voices.length > 0) {
              ...
              if (femaleVoice) {
                ...
                utterance.voice = voices[1]; // Use second voice if available
              } else {
                utterance.voice = voices[0]; // Use first voice
              }
            }
            ...
            console.log('Voice notification triggered');
        ...
        setError("Unexpected response: token not found. Please try again.");
        console.warn('Login response shape:', response.data);
      }
    } catch (err) {
